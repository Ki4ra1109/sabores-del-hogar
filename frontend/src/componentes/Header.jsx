...
export const Header = () => {
  ...
  const buscadorDesktopRef = useRef(null);
  const buscadorMobileRef = useRef(null);
  ...
  const ButtonWithLoader = ({
    ...
    >
      <span style={{ opacity: busy ? 0 : 1, transition: "opacity .15s" }}>
        {label}
      </span>
      {busy && (
  ...
  useEffect(() => {
    const onDocClick = (e) => {
      const insideDesktop =
        buscadorDesktopRef.current &&
        buscadorDesktopRef.current.contains(e.target);
      const insideMobile =
        buscadorMobileRef.current &&
        buscadorMobileRef.current.contains(e.target);
  ...
  const onAuthPanelKeyDown = (e) => {
    ...
    const nodes = Array.from(
      authPanelRef.current.querySelectorAll(selectors)
  ...
  const onLoginSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.message || "Email o contraseña incorrecta");
      }
  ...
  const sendCode = async () => {
    ...
    try {
      ...
      if (!r.ok) {
        throw new Error(d?.message || "No se pudo enviar el código");
      }
      setFgMsg("Código enviado. Revisa tu correo.");
      setFgStep(1);
      localStorage.setItem(
        FG_STORE,
        JSON.stringify({ email: fgEmail, step: 1 })
      );
    } catch (e) {
  ...
  return (
    <header>
      <div className="mensaje-banner">
        <p>
          🎉 Bienvenido a mi Sabores del Hogar Ofertas especiales todo el mes 🎉
        </p>
      </div>
      ...
      <nav className="Header-nav">
        ...
        <div className="Header-search Header-search--desktop">
          <SearchBox formRef={buscadorDesktopRef} />
        </div>

        <div className="Header-right">
          {!hideQuickAuth && (
            <div
              className={`auth-popover ${authOpen ? "open" : ""}`}
              ref={authRef}
            >
              <button
                type="button"
                    ...
                    {authMode === "login" && (
                      <>
                          ...
                          <label>
                            ...
                            <div className="pwd-wrap">
                              ...
                              >
                                <FaEyeSlash
                                  style={{
                                    display: showPwd ? "inline" : "none",
                                  }}
                                />
                                <FaEye
                                  style={{
                                    display: showPwd ? "none" : "inline",
                                  }}
                                />
                              </button>
                            </div>
                        ...
                        <p className="auth-register-text">
                          Si no tienes una cuenta registrate aca
                        </p>
      ...
      <div className="Header-search Header-search--mobile">
        <SearchBox formRef={buscadorMobileRef} />
      </div>
        ...
        <ul className="navbar-list">
          ...
          >
            ...
            <ul className="submenu">
              ...
              <li>
                <Link to="/postre" onClick={() => setMenuOpen(false)}>
                  Arma tu Postre
                </Link>
              </li>
            </ul>
        ...
        >
          ...
          >
            ...
            <ul className="mobile-menu-list">
              ...
              <li>
                <Link to="/postre" onClick={() => setMobileMenuOpen(false)}>
                  Arma tu Postre
                </Link>
              </li>
