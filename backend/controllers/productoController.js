...
exports.crearProducto = async (req, res) => {
  try {
    ...
    try {
      ...
      // manejar error de constraint (sku duplicado)
      console.error("Insert error:", err);
      if (err && err.original && err.original.code === "23505") {
        return res.status(400).json({ ok: false, message: "El SKU ya existe. Elige otro." });
      }
      throw err;
    }
  } catch (error) {
...
exports.actualizarProducto = async (req, res) => {
  try {
    ...
    // Si recibimos data URI en imagen, guardarla en disco y usar su ruta pública
    let imgPath = img;
    if (imgPath && imgPath.startsWith("data:")) {
      try {
        imgPath = await saveDataUriToFile(imgPath, sku);
